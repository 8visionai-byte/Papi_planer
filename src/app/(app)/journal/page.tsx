  ...
  const copyMarkdown = async () => {
    ...
    try {
      if (navigator.clipboard?.writeText) {
        ...
        showToast("Markdown skopiowany do schowka");
      } else {
        // Fallback
      ...
      {!loading && entries.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {entries.map((e) => {
            ...
            const display = (e.redactedText ?? e.rawText).trim();
            return (
              <div key={e.id} style={cardStyle}>
                ...
                >
                  ...
                  {e.category && (
                    <Badge label={e.category} palette={CATEGORY_COLORS[e.category]} />
                  )}
                  {e.topic && (
                    <Badge label={e.topic} palette={TOPIC_COLORS[e.topic]} />
                  )}
                  {!e.redactedText && (
                    <Badge
                      label="surowy"
