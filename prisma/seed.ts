...
// Mentor definitions live in their own side-effect-free module: this file calls main()
// at load, so anything importing FROM here would run the whole seed.
...
async function seedMentors(userId: string, areas: Record<string, string>) {
  for (const def of MENTOR_DEFS) {
    await upsertMentor(prisma, def, userId, areas);
  }
}
